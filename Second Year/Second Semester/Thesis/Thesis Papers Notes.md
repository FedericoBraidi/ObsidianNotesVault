...
## Conclusions
# 6. Abstract representations emerge naturally in neural networks trained to perform multiple tasks.
## Main idea
It has been proven that RNNs trained on multiple tasks simulatneously learn task representations based on simpler subtasks. Here it is shown that this emerges because of the similarity of the tasks and the fact that they are trained simulatenously. It is also shown that this process allows for few shot learning on novel tasks.
## Experiments
An example of these latent variables that help generalization is related to berries. We can describe a berry based on two latent variables: shape and color. We can train only on one side of one of these variables and check if the model generalizes to the other side. To measure this generalization we can use a metric of classification (meaning, does the boundary learned by a classification model also generalize to the unseen space?) or a metric of regression (meaning, does a linear fit done on the half space also generalize to the other half?).

The objective is to demonstrate that fact of learning multiple tasks simultaneously refines non-abstract representations into abstract ones. To do so, we start with abstract representations of dimension 5 and artificially increase their dimension using an upscaling autoencoder that makes the dimension 200. This makes it so that the representation is not abstract any longer, as can be seen by constructing sets of points that draw concentric squares in the original 5 dim space and then plotting the top 3 PCs of their 200 dim representation, which shows that the structure (and so the abstractness) is gone completely.

![[Pasted image 20250318124950.png]]

As expected both metrics (the classification generalization one and the regression one) show that the abstraction has been completely lost. The following graph shows on each graph the performance of the model trained on the whole space or the one trained on half and tested on the other half. 

![[Pasted image 20250318125118.png]]

They now use these non abstract feature representations as inputs of a multi layer perceptron which is trained to perform many tasks. These tasks are all binary classifications where the dividing hyperplane is not perpendicular to any of the input features (meaning there isn’t explicitly a given feature for which the answer is just the sign of it).
